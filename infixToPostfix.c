...
isOperand(s):
    1.return (s>= 'a' && s<= 'z') || (s>= 'A' && s<= 'Z')||(s>= '0' && s<= '9')
...
infixToPostfix(infix):
    ...
    3.while infix[i]!='\0' :
        3.1.token = infix[i]
        3.2.if isOperand(token):
            3.2.1 postfix[j++]=token
        3.3.else if token=='(':
            3.3.1 push(token)
        3.4.else if token==')':
            ...
            3.4.2 pop()
        3.5.else if isOperator(token):
            3.5.1 while top!=-1 && precedence(peek())>=precedence(token):
                3.5.1.1 postfix[j++]=pop()
            3.5.2 push(token)
        3.6.i++
    4.while top!=-1:
        4.1 postfix[j++]=pop()
